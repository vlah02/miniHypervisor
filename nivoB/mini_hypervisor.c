...
int init_hypervisor(struct hypervisor* hypervisor) {
    ...
    if (hypervisor->kvm_run_mmap_size < 0) {
        ...
        close(hypervisor->kvm_fd); // Cleanup
        return -1;
    }
...
int create_memory_region(struct guest* vm, size_t mem_size) {
    ...
    if (ioctl(vm->vm_fd, KVM_SET_USER_MEMORY_REGION, &region) < 0) {
        ...
        munmap(vm->mem, mem_size); // Cleanup
        return -1;
    }
...
int init_guest(struct hypervisor* hypervisor, struct guest* vm, size_t mem_size, enum PageSize page_size, FILE* img) {
    ...
    while (!feof(img)) {
        int r = fread(p, 1, BUFFER_SIZE, img);
        p += r;
    }
...
/**
 * Starts the guest VM in a new thread.
 *
 * @param arg Pointer to the guest structure.
 * @return NULL on completion.
 */
void* start_guest_thread(void* arg) {
    struct guest* vm = (struct guest*)arg;
    run_guest(vm);
    return NULL;
}
...
int main(int argc, char* argv[]) {
    ...
    char** guest_files = NULL; // Array of guest image file paths
    int num_guests = 0; // Number of guest VMs
    ...
    while ((opt = getopt_long(argc, argv, "m:p:g:", long_options, NULL)) != -1) {
        switch (opt) {
            ...
            case 'p':
                ...
                break;
            case 'g':
                guest_files = &argv[optind - 1]; // Set the guest image files
                num_guests = argc - optind + 1; // Calculate the number of guest VMs
                break;
            default:
                fprintf(stderr, "Usage: %s --memory <MB> --page <4|2> --guest <guest.img> [<guest2.img> ...]\n", argv[0]);
                exit(EXIT_FAILURE);
        }
    }

    // Check if the guest files were provided
    if (guest_files == NULL) {
        fprintf(stderr, "ERROR: No guest file provided.\n");
        exit(EXIT_FAILURE);
    }
    ...
    pthread_t threads[num_guests]; // Array of thread identifiers
    struct guest vms[num_guests]; // Array of guest VM structures

    for (int i = 0; i < num_guests; i++) {
        // Open the guest image file
        FILE* img = fopen(guest_files[i], "r");
        if (img == NULL) {
        ...
        if (init_guest(&hypervisor, &vms[i], memory, page_size, img) < 0) {
        ...
        if (pthread_create(&threads[i], NULL, start_guest_thread, &vms[i]) != 0) {
            perror("ERROR: Failed to create thread for guest");
            fclose(img); // Cleanup
            exit(EXIT_FAILURE);
        }
